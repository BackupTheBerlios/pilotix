...
public class ClientHandlerThread extends Thread {
	...
	public ClientHandlerThread(int theClientId, MessageHandler aMessageHandler) throws Exception {
			...
			try {
				...
				if (flag == Transferable.COMMAND) {
					...
					if (info.getType() == Information.DECONNECT) {
						...
						// messageHandler.close(); // Entraine une exception
						// "Socket closed"
						// dans le serveur
						status = WANTTOLEAVE;
				...
				quit = true;
				System.out.println("[ClientHandlerThread]  Ship " + ship.getId() + " Has gone !");
				try {
					PilotixServer.theIH.giveBackId(ship.getId());
				} catch (Exception f) {
					f.printStackTrace();
