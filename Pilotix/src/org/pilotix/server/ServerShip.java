...
public class ServerShip extends Ship {
	...
	public void set(int aShipId, Vector aPosition, ServerAngle aDirection, int theStates) {
		id = aShipId;
		states = theStates;
	...
	public void addCommand(Command aCommand) {
		tmpAngle.set((command.getDirection()).floatValue() + ((aCommand.getDirection()).floatValue()));
		command.setDirection(tmpAngle);
	...
	public void computeSpeedFromForces() {
		...
		tmpVector.set((int) (((ServerAngle) direction).getX() * command.getAcceleration() * 10), (int) (((ServerAngle) direction).getY() * command.getAcceleration() * 10));
	...
	public void commandPilotixElement(IterableArray balls) {
		if (command.getToolId() == 1) {
			command.setToolId(0);
			if (balls.isNull(id)) {
				balls.add(id, new ServerBall(id, position.plus((int) (((ServerAngle) direction).getX() * (double) radius), (int) (((ServerAngle) direction).getY() * (double) radius)), speed.plus((int) (((ServerAngle) direction).getX() * (double) 200), (int) (((ServerAngle) direction).getY() * (double) 200))));
			} else {
