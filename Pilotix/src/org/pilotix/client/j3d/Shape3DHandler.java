...
public class Shape3DHandler {
	 ...
	 * dans le répertoire data/shapes. Vous ne pouvez utiliser cette méthode que
	 * si
	 * 
	 * @param aShapeFile
	 *            nom du fichier XML à traiter
	 * @return l'objet Shape3D
	 */
	...
	public Shape3D getShape3DFromURL(String aShapeFile, Color3f aDynamicColor) {
		document = Environment.theXMLHandler.getDocumentFromURL(Environment.theRL.getResource(ResourceLocator.SHAPE, aShapeFile));
		...
		for (int i = 0; i < faceLists.getLength(); i++) {
			NodeList faces = ((Element) faceLists.item(i)).getElementsByTagName("Face");
			count = count + faces.getLength();
		}
		if (Environment.debug) {
			System.out.println("[Shape3DHandler.getShape3DFromURL(" + aShapeFile + ")] Total faces : " + count);
		}
		TriangleArray triangleArray = new TriangleArray(count * 3, GeometryArray.COORDINATES | GeometryArray.NORMALS | GeometryArray.COLOR_3);
		...
		for (int i = 0; i < faceLists.getLength(); i++) {
			...
			for (int j = 0; j < faces.getLength(); j++) {
				...
				while (pointsIds.hasMoreTokens()) {
					xmlPoint = (Element) (points.item(Integer.parseInt(pointsIds.nextToken())));
					triangleArray.setCoordinate(pointCount, new Point3f(Float.parseFloat(xmlPoint.getAttribute("X")), Float.parseFloat(xmlPoint.getAttribute("Y")), Float.parseFloat(xmlPoint.getAttribute("Z"))));
