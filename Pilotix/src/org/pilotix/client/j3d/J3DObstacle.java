...
import javax.media.j3d.TransparencyAttributes; //import javax.media.j3d.PolygonAttributes;
...
public class J3DObstacle extends J3DObject {
	 ...
	 * ResourceLocator.
	 */
	public J3DObstacle(Vector upLeftCorner, Vector downRightCorner, int height, int altitude, String topTexture, String sideTexture) {

		super();

		// --------------------
		// GEOMETRIE : on réserve 4*nbFaces points
		QuadArray quadArray = new QuadArray(4 * nbFaces, GeometryArray.COORDINATES | GeometryArray.TEXTURE_COORDINATE_2 | GeometryArray.NORMALS | GeometryArray.COLOR_3);
		...
		Point3f pointBasGaucheAuSol = new Point3f(0.0f, 0.0f, 0.0f);
		Point3f pointBasGaucheEnAlt = new Point3f(0.0f, 0.0f, height * Environment.u3d);
		Point3f pointBasDroiteAuSol = new Point3f((downRightCorner.x - upLeftCorner.x) * Environment.u3d, 0.0f, 0.0f);
		Point3f pointBasDroiteEnAlt = new Point3f((downRightCorner.x - upLeftCorner.x) * Environment.u3d, 0.0f, height * Environment.u3d);
		Point3f pointHautDroiteAuSol = new Point3f((downRightCorner.x - upLeftCorner.x) * Environment.u3d, (upLeftCorner.y - downRightCorner.y) * Environment.u3d, 0.0f);
		Point3f pointHautDroiteEnAlt = new Point3f((downRightCorner.x - upLeftCorner.x) * Environment.u3d, (upLeftCorner.y - downRightCorner.y) * Environment.u3d, height * Environment.u3d);
		Point3f pointHautGaucheAuSol = new Point3f(0.0f, (upLeftCorner.y - downRightCorner.y) * Environment.u3d, 0.0f);
		Point3f pointHautGaucheEnAlt = new Point3f(0.0f, (upLeftCorner.y - downRightCorner.y) * Environment.u3d, height * Environment.u3d);
		...
		TextureLoader loader = new TextureLoader(Environment.theRL.getResource(ResourceLocator.TEXTURE, topTexture), Environment.theGUI);
		ImageComponent2D topImage = loader.getImage();
		Texture2D texture2D = new Texture2D(Texture.BASE_LEVEL, Texture.RGB, topImage.getWidth(), topImage.getHeight());
		...
		float x_sur_y = ((downRightCorner.x - upLeftCorner.x) * Environment.u3d) / ((downRightCorner.y - upLeftCorner.y) * Environment.u3d);
		int nbRepetitions = 1;
		...
		this.setPosition(new Vector(upLeftCorner.x, downRightCorner.y));
