...
class ClientTest {
		...
		loopThread = new Thread() {
			...
			public void run() {
				...
				while (!quit) {
					try {
						/*
						 * dummy = msHandler.receive(); if (dummy instanceof
						 * Information) System.out.println("Recu une info");
						 * else if (dummy instanceof Command)
						 * System.out.println("Recu une commande");
						 * msHandler.send(cmd);
						 */
	...
	public static void main(String[] args) {
		if (args.length != 2)
			...
			new ClientTest(args[0], Integer.parseInt(args[1]));
		}
	}
}
