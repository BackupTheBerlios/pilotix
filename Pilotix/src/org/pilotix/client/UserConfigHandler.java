 ...
 * de récupérer les champs contenus dans ce fichier. Les champs obligatoires
 * (tous ceux qui ne correspondent pas aux plugins) sont recopiés dans des
 * variables locales. Ces valeurs sont recopiées à nouveau dans le Document lors
 * de la sauvegarde.
 * 
 * @author Loïc Guibart
 */
public class UserConfigHandler {
	...
	/*
	 * private HashMap builtInVars; private HashMap plugInVars;
	 */
	...
	public UserConfigHandler(String userName) {
		configDocument = Environment.theXMLHandler.getDocumentFromURL(Environment.theRL.getResource(ResourceLocator.PILOT, userName + cfgFileName));
	...
	public void setKeymap(HashMap<String,String> aKeymap) {
		keymap = aKeymap;
	}
	 ...
	 * @param builtInName
	 *            Nom de la section.
	 * @return HashMap contenant les associations nom-valeur des variables de la
	 *         section.
	 */
	/*
	 * public HashMap getBuiltInVars(String builtInName) { Element elt =
	 * getElementByTypeAndName(builtInString, builtInName); if (elt == null)
	 * return null; else return getVarsFromElement(elt); }
	 */
	 ...
	 *         section.
	 */
	public HashMap<String,String> getPlugInVars(String plugInName) {
	 ...
	 * NodeList nl = documentElement.getElementsByTagName(type); if
	 * (nl.getLength() == 0) return null; else { Element elt = null; int i = 0;
	 * boolean found = false; while (!found && i < nl.getLength()) { elt =
	 * (Element) nl.item(i); if (elt.getAttribute("name").equals(name)) found =
	 * true; i++; } if (found) return elt; else return null; } }
	 */
	 ...
	 * 
	 * <pre>
	 * <tagName name="nom">valeur</tagName>
	 * </pre>
	 * 
	 * .
	 * 
	 * @param topElement
	 *            Element père des éléments concernés.
	 * @param tagName
	 *            Nom des éléments concernés.
	 * @return HashMap contenant les associations nom-valeur des éléments
	 *         concernés.
	 */
	private HashMap<String, String> getValuesFromElements(Element topElement, String tagName) {
		NodeList nl = topElement.getElementsByTagName(tagName);
		HashMap<String, String> result = new HashMap<String, String>(nl.getLength(), 1);
		Element elt;
		for (int i = 0; i < nl.getLength(); i++) {
			elt = (Element) nl.item(i);
			result.put(elt.getAttribute("name"), elt.getFirstChild().getNodeValue());
		}
		return result;
	}
}
