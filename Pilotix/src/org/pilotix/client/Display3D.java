...
public class Display3D {
	...
	public void init() {
		...
		objectsJ3D.put("J3DMinimap", new J3DMinimap(minimapCanvas3D,
				xMax / 2.0f, yMax / 2.0f));
		...
		objectsJ3D.put("J3DArea", new J3DArea(Environment.theClientArea
				.getXMax(), Environment.theClientArea.getYMax()));
		...
		for (int i = 0; i < Environment.theClientArea.getObstacles().length; i++) {
			...
			objectsJ3D
					.put("obstacle" + i, new J3DObstacle(upLeftCorner,
							downRightCorner, height, altitude, topTexture,
							sideTexture));
	...
	public void reset() {
		if (objectsJ3D.containsKey("J3DMinimap")) {
			((J3DMinimap) objectsJ3D.get("J3DMinimap")).getCamera().getView()
	...
	private void addShip(int aShipId, Vector aPosition, Angle aDirection,
		...
		if (objectsJ3D.get("ship" + aShipId) == null) {
			...
			objectsJ3D.put("ship" + aShipId, new J3DShip(
					"wipeout.pilotix.shape.xml", thisShipColor));
			...
			if (Environment.theClientArea.getOwnShipId() == aShipId) {
				...
				((J3DShip) objectsJ3D.get("ship" + aShipId))
						.addCamera(ownShip3DCamera);
			}

			// Ajout du J3DShip dans la locale
			locale.addBranchGraph((BranchGroup) objectsJ3D
					.get("ship" + aShipId));
	...
	private void removeShip(int aShipId) {
		if (aShipId == Environment.theClientArea.getOwnShipId()) {
			...
			((J3DShip) objectsJ3D.get("ship"
					+ Environment.theClientArea.getOwnShipId())).getCamera()
					.getView().removeCanvas3D(mainCanvas3D);
		}
		// On retire une branche de la locale.
		locale
				.removeBranchGraph((BranchGroup) objectsJ3D.get("ship"
						+ aShipId));
	...
	private void updateShip(int aShipId) {
		...
		if (!Environment.theClientArea.shipIsNull(aShipId)) {
			if (objectsJ3D.containsKey("ship" + aShipId)) {
				...
				setShip(aShipId, Environment.theClientArea
						.getShipPosition(aShipId), Environment.theClientArea
						.getShipDirection(aShipId), 100,
						Environment.theClientArea.getShipStates(aShipId));
			} else {
				// ...sinon si seul le J3DShip est null, on ajoute ce vaisseau
				addShip(aShipId, Environment.theClientArea
						.getShipPosition(aShipId), Environment.theClientArea
						.getShipDirection(aShipId), 100);
			}
	...
	public void update() {
		/*
		 * for(Environment.theClientArea.getShips().setCursorOnFirst();
		 * Environment.theClientArea.getShips().hasNext();){
		 *  }
		 */
