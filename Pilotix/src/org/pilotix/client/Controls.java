...
public class Controls extends KeyAdapter implements KeyEventDispatcher {
	...
	/**
	 * Nombre de touches sur le clavier le nombre est volontairement trop grand.
	 */
	...
	/**
	 * état de la touche : pressée et relachée depuis le dernier appel à
	...
	/**
	 * Indique à <code>Controls</code> où doivent être récupérés les évènements
	 * claviers.
	 * 
	 * @param gui
	 ...
	 * configuration utilisateur. Le code touche correspond à une constante
	 * <code>VK_*</code> définie dans la classe <code>KeyEvent</code>.
	 * 
	 * @param actionName
	 *            Nom de l'action.
	...
	public int getKeyCodeFromAction(String actionName) {
		String actionKey = Environment.userConfig.getKeymap().get(actionName);
	...
	/**
	 * Effectue la correspondance entre un intutilé de touche et son code (voir
	 * KeyEvent). Malheureusement cette méthode n'existe pas dans KeyEvent... Je
	 * n'ai trouvé que ce moyen de faire mais c'est pas terrible...
	 * 
	 * @param touche
	 * @return code correspondant.
	 */
	...
	class Mouse extends MouseInputAdapter implements MouseWheelListener {
		...
		public Mouse() {
			...
			hideCursor = Toolkit.getDefaultToolkit().createCustomCursor(image, new Point(0, 0), "hidden_cursor");
		}
		...
		public Vector getMouseVariation() {
			...
			if (grabbed) {
				mouseMover.mouseMove(xCenter + windowPos.x, yCenter + windowPos.y);
