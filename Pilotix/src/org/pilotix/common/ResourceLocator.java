...
public class ResourceLocator {
	...
	public final static int CONFIG = 0, TEXTURE = 1, SHIP = 2, SHAPE = 3, AREA = 4, PILOT = 5;
	...
	/**
	 * Cette méthode renvoie l'URL de la ressource dont le type et le nom sont
	 * fournis. La recherche est faite prioritairement sur le disque, puis en
	 * cas d'échec, dans le fichier Jar.
	 * 
	 * @param resourceType
	 *            le type du fichier
	 * @param resourceName
	 *            le nom du fichier avec son extension
	 * @return l'URL du fichier, ou null s'il est introuvable
	 */
	public URL getResource(int resourceType, String resourceName) {
		...
		case TEXTURE:
			...
			if (resURL == null) {
				resURL = thisClassLoader.getResource(texturesPath + resourceName);
				// if (Environment.debug) {
		...
		case SHIP:
			...
			if (resURL == null) {
				resURL = thisClassLoader.getResource(shipsPath + resourceName);
				// if (Environment.debug) {
		...
		case SHAPE:
			...
			if (resURL == null) {
				resURL = thisClassLoader.getResource(shapesPath + resourceName);
				// if (Environment.debug) {
